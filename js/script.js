...
$(document).ready(function() {
    ...
    $("#novidadesform [type='submit']").click(function(e) {
        ...
        if(valor == ""){
            ...
            // alert('Informe um email seu fuleragi.');
        }
        else{
            $.ajax({
              type: 'post',
              ...
              success: function(retorno){
                $("#email").val('');
                $(".resultado").text("*emaildigitado* foi salvo em nossa lista de novidades =)");
                $('#alerta').fadeOut().delay(2000);
              },
              error: function(chiclete){
                console.log(chiclete);
                toastr.error(chiclete.responseJSON.text, 'Error!');
              }
            });
        }
